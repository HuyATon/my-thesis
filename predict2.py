...
with torch.no_grad():
    for mask_folder in mask_folders:
        ...
        for index, (inputs, _) in enumerate(test_loader):  
            ...
            masks = torch.nn.functional.interpolate(masks, size=(256, 256), mode="nearest")

            # Forward pass
            outputs = model(imgs, masks)

            for i in range(outputs.shape[0]):
                out = torch.clamp(outputs[i], -1, 1) 
                out = (out + 1) / 2  
                inpainted_img = (out.permute(1, 2, 0).cpu().numpy() * 255).astype(np.uint8)

                original_filename = test_dataset.imgs[index * args.batch_size + i]
                save_path = os.path.join(out_dir[mask_folder], original_filename)
